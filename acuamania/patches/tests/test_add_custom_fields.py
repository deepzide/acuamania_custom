...
class TestAddCustomFieldsPatch(FrappeTestCase):
    ...
    def test_field_definitions_are_consistent_with_json(self):
        ...
        for doctype, fields in self.custom_fields_data.items():
            for field in fields:
                ...
                self.assertEqual(
                    db_field["label"], field.get("label"), f"Label mismatch for {field['fieldname']} in {doctype}."
                )
                self.assertEqual(
                    db_field["fieldtype"],
                    field.get("fieldtype"),
    ...
    def test_patch_is_idempotent(self):
        ...
        self.assertEqual(initial_counts, second_counts, "Patch should be idempotent and not duplicate fields.")
