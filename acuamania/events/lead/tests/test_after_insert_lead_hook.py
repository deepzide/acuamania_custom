...
class TestLeadContactCreation(FrappeTestCase):
		...
		for doctype, name in self.created_docs:
			if frappe.db.exists(doctype, name):
				try:
					...
					frappe.log_error(
						f"Error deleting {doctype} {name}: {e}", "TestLeadContactCreation Cleanup Error"
					)
		frappe.db.commit()
	...
	def test_contact_created_when_lead_inserted(self):
		...
		contact_name = frappe.db.get_value("Contact", {"phone": "099111222"}, "name") or frappe.db.get_value(
			"Contact", {"custom_phone": "099111222"}, "name"
		)
