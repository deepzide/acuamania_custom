...
def _calculate_split_amounts(outstanding):
	...
	frappe.msgprint(f"[AUTO-PE] Split calculated → deposit={deposit}, balance={balance}")
	return {
		"deposit": deposit,
		"balance": balance,
	}
...
def _create_payment_entry(invoice, params):
	...
	payment_entry.references[0].allocated_amount = flt(params["allocated_amount"])
	...
	frappe.msgprint(f"[AUTO-PE] Payment Entry {payment_entry.name} created (Draft)")
