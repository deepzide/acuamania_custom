...
class TestPromoEngine(FrappeTestCase):
	...
	def test_requeridos_x_gratuitos_even(self):
		self.run_promo_test("Quotation", "ONFI 2x1", qty=4, rate=910, expected=1820)

	def test_requeridos_x_gratuitos_odd(self):
		self.run_promo_test("Quotation", "ONFI 2x1", qty=3, rate=910, expected=910)

	def test_fixed_price(self):
		expected = (910 - 610) * 2
		self.run_promo_test(
			"Quotation", "Reencuentro - Todos Somos Residentes", qty=2, rate=910, expected=expected
		)

	def test_percentage_discount(self):
		self.run_promo_test("Quotation", "Descuento 10%", qty=2, rate=1000, expected=200)

	def test_discount_amount(self):
		self.run_promo_test("Quotation", "Descuento Fijo 500 UYU", qty=2, rate=900, expected=500)
	...
	def test_so_requeridos_x_gratuitos_odd(self):
		self.run_promo_test("Sales Order", "ONFI 2x1", qty=3, rate=910, expected=910)

	def test_so_fixed_price(self):
		expected = (910 - 610) * 2
		self.run_promo_test(
			"Sales Order", "Reencuentro - Todos Somos Residentes", qty=2, rate=910, expected=expected
		)

	def test_so_percentage_discount(self):
		self.run_promo_test("Sales Order", "Descuento 10%", qty=2, rate=1000, expected=200)

	def test_so_discount_amount(self):
		self.run_promo_test("Sales Order", "Descuento Fijo 500 UYU", qty=2, rate=900, expected=500)
