...
def test_append_creates_buffer():
    ...
    r = requests.post(
        f"{BASE_URL}{ENDPOINT}",
        data={"phone": CONTACT_PHONE, "message": "Hola"},
        headers=api_headers()
    )
    ...
    contact = requests.get(
        f"{BASE_URL}/api/resource/Contact?filters={{\"phone\":\"{CONTACT_PHONE}\"}}",
        headers=api_headers()
    ).json()
    ...
    # Fetch full doc
    doc = requests.get(
        f"{BASE_URL}/api/resource/Contact/{docname}",
        headers=api_headers()
    ).json()
...
def test_ignore_if_no_contact():
    r = requests.post(
        f"{BASE_URL}{ENDPOINT}",
        data={"phone": "00000000", "message": "Hola"},
        headers=api_headers()
    )
